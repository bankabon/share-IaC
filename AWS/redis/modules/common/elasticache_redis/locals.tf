locals {
  ...
  cluster_name               = var.cluster_name != null ? var.cluster_name : local.name_kebab
  security_group_name        = var.security_group_name != null ? var.security_group_name : local.name
  security_group_description = var.security_group_description != null ? var.security_group_description : "Redis cluster (${local.name})"
  subnet_group_name          = var.subnet_group_name != null ? var.subnet_group_name : local.name_kebab
  subnet_type                = var.subnet_type != null ? var.subnet_type : "cache"
